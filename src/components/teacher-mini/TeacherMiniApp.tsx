...
function AttendanceScreen({ notify, onProfileMissing, onLessonSubmitted, navigationTarget }: { notify: (message: string, tone?: ToastTone) => void; onProfileMissing: () => void; onLessonSubmitted?: () => void; navigationTarget?: LessonNotificationTarget | null }) {
  ...
  const now = useMinuteClock();
  const today = toISO(new Date());
  const range = section === "today" ? { from: today, to: today } : { from: toISO(new Date(month.getFullYear(), month.getMonth(), 1)), to: toISO(new Date(month.getFullYear(), month.getMonth() + 1, 0)) };

  useEffect(() => { if (!navigationTarget?.date) return; const targetDate = fromISO(navigationTarget.date); setMonth(new Date(targetDate.getFullYear(), targetDate.getMonth(), 1)); setSection(navigationTarget.date === today ? "today" : "history"); }, [navigationTarget?.date, today]);
...
export function NotificationSheet({ onClose, notify, onCountChange, onNavigateLessonReport, onNavigateScheduleConfirmation }: { onClose: () => void; notify: (message: string, tone?: ToastTone) => void; onCountChange: (count: number) => void; onNavigateLessonReport?: (target: LessonNotificationTarget) => void; onNavigateScheduleConfirmation?: (target: ConfirmationNavigationTarget) => void }) {
  ...
  useEffect(() => onTeacherNotification((item) => { if (tab === "unread") setItems((current) => current.some((entry) => entry.id === item.id) ? current : [item, ...current]); }), [tab]);
  const openItem = async (item: TeachingNotification) => {
    try {
      const unreadItem = tab === "unread" || item.isRead === false;
      if (unreadItem) { await teacherMiniApi.notifications.read(item.id); setItems((current) => current.filter((entry) => entry.id !== item.id)); onCountChange(-1); }
      const meta = item.meta ?? item.metadata;
      const type = item.type || meta?.kind || "";
      const lessonAlert = type === "TEACHING_LESSON_REPORT_ALERT" || type === "lesson_report_missing";
  ...
  const readAll = async () => { try { await teacherMiniApi.notifications.readAll(); setItems([]); onCountChange(-9999); notify("Đã đánh dấu tất cả là đã đọc"); } catch (error) { explainError(error, notify); } };
  return <Sheet className="notification-sheet" title={<span className="notification-title"><NotificationIcon name="bell" /> Thông báo</span>} onClose={onClose}><div className="notification-tabs" role="tablist"><button role="tab" aria-selected={tab === "unread"} className={tab === "unread" ? "active" : ""} onClick={() => setTab("unread")}>Chưa đọc{tab === "unread" && items.length > 0 && <b>{items.length}</b>}</button><button role="tab" aria-selected={tab === "read"} className={tab === "read" ? "active" : ""} onClick={() => setTab("read")}>Đã đọc</button>{tab === "unread" && items.length > 0 && <button className="read-all-button" onClick={readAll}>Đọc tất cả</button>}</div>{loading ? <LoadingCards /> : <div className="notification-list">{items.map((item) => { const isUnread = tab === "unread" || item.isRead === false; const presentation = notificationPresentation(item); return <button type="button" className={`${isUnread ? "unread" : ""} ${presentation.className}`} key={item.id} onClick={() => openItem(item)}><span className="notification-card-icon"><NotificationIcon name={presentation.icon} /></span><span className="notification-card-copy"><b>{presentation.title}</b><p>{item.message || item.content || "Bạn có một cập nhật mới về lịch dạy."}</p><small><NotificationIcon name="clock" size={11} /> {item.createdAt ? new Date(item.createdAt).toLocaleString("vi-VN") : "Vừa xong"}</small></span>{isUnread && <span className="notification-dot" aria-label="Chưa đọc" />}</button>; })}{!items.length && <div className="notification-empty"><span><NotificationIcon name="bell" size={28} /></span><b>{tab === "unread" ? "Bạn đã đọc hết thông báo" : "Chưa có thông báo đã đọc"}</b><p>Các cập nhật lịch dạy mới sẽ xuất hiện tại đây.</p></div>}</div>}</Sheet>;
}
