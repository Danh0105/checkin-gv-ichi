...
const gasAllowanceText = (session: Pick<TeachingSession, "gasAllowance" | "distanceToSchoolKm">) => session.gasAllowance == null
  ? "Chưa có phụ cấp xăng"
  : session.distanceToSchoolKm == null ? `${session.gasAllowance.toLocaleString("vi-VN")} ₫` : `${session.gasAllowance.toLocaleString("vi-VN")} ₫ · ${session.distanceToSchoolKm.toLocaleString("vi-VN")} km`;
const compensationText = (session: TeachingSession, companyTeacher = false) => companyTeacher ? gasAllowanceText(session) : money(session.amount);
...
function NotificationIcon({ name, size = 22 }: { name: "bell" | "calendar" | "clock" | "home" | "logout" | "refresh"; size?: number }) {
  const paths: Record<"bell" | "calendar" | "clock" | "home" | "logout" | "refresh", ReactNode> = {
...
function cellStatusBadges(session: TeachingSession): CellBadge[] {
  ...
  if (session.assignmentStatus === "ASSIGNED") badges.push(session.checkoutAt ? { key: "out", label: "Đã check-out", tone: "done" } : { key: "out", label: "Chưa check-out", tone: "pending" });
...
export function SessionDetailSheet({ session, onClose, onUpdate, onDeclined, onRefresh, onLessonSubmitted, notify, companyTeacher = false }: { session: TeachingSession; onClose: () => void; onUpdate: (session: TeachingSession) => void; onDeclined: (session: TeachingSession) => void; onRefresh?: (sessionId: number, mutation?: TeachingSession) => Promise<void>; onLessonSubmitted?: () => void; notify: (message: string, tone?: ToastTone) => void; companyTeacher?: boolean }) {
  ...
  return <><Sheet title="Chi tiết buổi dạy" onClose={onClose}><div className="mini-session-detail">
    ...
    <dl><div><dt>Trường</dt><dd>{session.schoolName}</dd></div><div><dt>Lớp</dt><dd>{session.className || "—"}</dd></div><div><dt>Số tiết</dt><dd>{periodsOf(session)} tiết</dd></div><div><dt>{companyTeacher ? "Phụ cấp xăng" : "Tiền dạy"}</dt><dd>{compensationText(session, companyTeacher)}</dd></div></dl>
...
export function ScheduleScreen({ notify, onProfileMissing, onLessonSubmitted, onConfirmationHandled, confirmationNavigation, companyTeacher = false, refreshKey = 0 }: { notify: (message: string, tone?: ToastTone) => void; onProfileMissing: () => void; onLessonSubmitted?: () => void; onConfirmationHandled?: () => void; confirmationNavigation?: ConfirmationNavigationTarget | null; companyTeacher?: boolean; refreshKey?: number }) {
  ...
  const summary = { sessions: actualSessions.length, periods: actualSessions.reduce((sum, item) => sum + periodsOf(item), 0), amount: actualSessions.reduce((sum, item) => sum + (item.amount ?? 0), 0), unpriced: !companyTeacher && actualSessions.some((item) => item.amount == null) };
  ...
  return <div className="teacher-screen">
    ...
    <div className="schedule-summary"><div><b>{summary.sessions}</b><span>Buổi thật</span></div><div><b>{summary.periods}</b><span>Số tiết</span></div><div><b>{companyTeacher ? "Xem từng tiết" : summary.unpriced ? "—" : summary.amount.toLocaleString("vi-VN")}</b><span>{companyTeacher ? "Phụ cấp xăng" : summary.unpriced ? "Có buổi chưa khai giá" : "Tổng tiền (₫)"}</span></div></div>
    ...
    {selected && <SessionDetailSheet session={selected} onClose={() => setSelected(null)} onUpdate={update} onDeclined={declined} onRefresh={refresh} onLessonSubmitted={onLessonSubmitted} notify={notify} companyTeacher={companyTeacher} />}
...
export function AttendanceScreen({ notify, onProfileMissing, onLessonSubmitted, navigationTarget, companyTeacher = false, refreshKey = 0 }: { notify: (message: string, tone?: ToastTone) => void; onProfileMissing: () => void; onLessonSubmitted?: () => void; navigationTarget?: LessonNotificationTarget | null; companyTeacher?: boolean; refreshKey?: number }) {
  ...
  useEffect(() => { setLoading(true); teacherMiniApi.sessions.me({ fromDate: range.from, toDate: range.to, limit: 200 }).then((result) => setSessions(result.data)).catch((error) => explainError(error, notify, onProfileMissing)).then(() => setLoading(false)); }, [range.from, range.to, refreshKey]);
...
function OpenSessionsScreen({ profile, notify, onProfileMissing, companyTeacher = false, refreshKey = 0 }: { profile: Teacher | null; notify: (message: string, tone?: ToastTone) => void; onProfileMissing: () => void; companyTeacher?: boolean; refreshKey?: number }) {
...
function shouldShowTeacherNotification(item: TeachingNotification, companyTeacher = true) {
  const meta = item.meta ?? item.metadata;
  const type = item.type || meta?.kind || "";
  return companyTeacher || type !== "TEACHER_LOCATION_CHANGE_RESULT";
}

export function NotificationSheet({ onClose, notify, onCountChange, onUnreadCountChange, onNavigateLessonReport, onNavigateScheduleConfirmation, companyTeacher = true }: { onClose: () => void; notify: (message: string, tone?: ToastTone) => void; onCountChange?: (count: number) => void; onUnreadCountChange?: (count: number) => void; onNavigateLessonReport?: (target: LessonNotificationTarget) => void; onNavigateScheduleConfirmation?: (target: ConfirmationNavigationTarget) => void; companyTeacher?: boolean }) {
  ...
  useEffect(() => onTeacherNotification((item) => {
    if (!shouldShowTeacherNotification(item, companyTeacher)) return;
...
export default function TeacherMiniApp({ user, initialTab = "schedule", onHome, onLogout }: { user: TeachingUser; initialTab?: MiniTab; onHome: () => void; onLogout: () => void }) {
  ...
  const refreshUnread = useCallback(() => {
    ...
    teacherMiniApi.notifications.list("unread").then((result) => {
      setUnread(result.data.filter((item) => shouldShowTeacherNotification(item, false)).length);
    }).catch(() => undefined);
  }, [companyTeacher]);
  ...
  useEffect(() => {
    const offNotification = onTeacherNotification((item) => {
      ...
      if (companyTeacher && item.type === "TEACHER_LOCATION_CHANGE_RESULT") notify(item.message || (item.meta?.approved === false ? "Yêu cầu đổi vị trí của bạn đã bị từ chối." : "Vị trí mới của bạn đã được duyệt."), item.meta?.approved === false ? "warning" : "success");
    });
